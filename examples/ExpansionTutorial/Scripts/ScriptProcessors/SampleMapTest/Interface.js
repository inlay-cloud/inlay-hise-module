...
inline function addRow(root, obj, isCategory)
{
    ...
    if(isCategory)
    {
        ...
        {
            // Fetch the name
            c.data.name = ExpansionProtection.getDisplayName(obj);
        }
        else
        {
...
{
    if (isDefined(cat.data.expansion)
        && !ExpansionProtection.requestAccess(cat.data.expansion))
        return;

    clearPanel(Panel2);

    if (isDefined(cat.data.expansion))
        cat.data.list = cat.data.expansion.getSampleMapList();
...
inline function refreshExpansionRows()
{
    clearPanel(Panel1);
    clearPanel(Panel2);

    // Root content is always available. Expansions without an Inlay product ID
    // are not listed; protected locked ones keep a visible unlock entry.
    addRow(Panel1, undefined, true);
...
inline function refreshActiveState()
{
    ...
    for(cat in Panel1.getChildPanelList())
    {
        ...
        // be selected
        if(isDefined(cat.data.list) && cat.data.list.contains(value))
            setCategory(cat);
...
ExpansionProtection.setRefreshCallback(refreshExpansionRows);
